...
var simpleAjaxHelper = __webpack_require__(4);
var mutualExclusiveTransactionHelper = __webpack_require__(5);

var jpipCodingPassesNumberParser = __webpack_require__(6);
var jpipMessageHeaderParser = __webpack_require__(7);

var JpipChannel = __webpack_require__(8);
var JpipCodestreamReconstructor = __webpack_require__(9);
var JpipCodestreamStructure = __webpack_require__(10);
var JpipComponentStructure = __webpack_require__(11);
var CompositeArray = __webpack_require__(12);
var JpipDatabinParts = __webpack_require__(13);
var JpipDatabinsSaver = __webpack_require__(14);
var JpipFetch = __webpack_require__(15);
var JpipHeaderModifier = __webpack_require__(16);
var JpipImageDataContext = __webpack_require__(17);
var JpipLevelCalculator = __webpack_require__(18);
var JpipMarkersParser = __webpack_require__(19);
...
var jpipRuntimeFactory = {
    ...
    createImageDataContext: function createImageDataContext(jpipObjects, codestreamPartParams, progressiveness) {
        ...
        return new JpipImageDataContext(jpipObjects, codestreamPartParams, progressiveness);
    },
    ...
    createObjectPoolByDatabin: function createObjectPoolByDatabin() {
...
JpipImageDataContext.prototype._tryAdvanceProgressiveStage = function tryAdvanceProgressiveStage() {
    ...
    while (this._progressiveStagesFinished < this._progressiveness.length) {
        var qualityLayersRequired = this._progressiveness[this._progressiveStagesFinished].minNumQualityLayers;

        if (qualityLayersRequired === 'max' || qualityLayersRequired > this._qualityLayersReached) {
...
module.exports = function JpipObjectPoolByDatabin() {
    ...
    this.getObject = function getObject(databin) {
        ...
        if (obj === undefined) {
            ...
            throw new jGlobals.jpipExceptions.InternalErrorException('Databin IDs are not unique');
        }
...
module.exports = function JpipPacketsDataCollector(databinsSaver, qualityLayersCache, jpipFactory) {
    ...
    function pushPackets(result, tileIndexInCodestreamPart, precinctIterator, precinctDatabin, returnedCodeblocksInPrecinct, quality) {
...
module.exports = function JpipRequestDatabinsListener(codestreamPart, qualityLayerReachedCallback, codestreamStructure, databinsSaver, qualityLayersCache, jpipFactory) {
    ...
    this.unregister = function unregister() {
        ...
        for (var i = 0; i < registeredTileHeaderDatabins.length; ++i) {
            databinsSaver.removeEventListener(registeredTileHeaderDatabins[i], 'dataArrived', tileHeaderDataArrived);
        }

        for (var j = 0; j < registeredPrecinctDatabins.length; ++j) {
            databinsSaver.removeEventListener(registeredPrecinctDatabins[j], 'dataArrived', precinctDataArrived);
        }
    ...
    function tileHeaderDataArrived(tileHeaderDatabin) {
        ...
        var tileAccumulatedData = accumulatedDataPerDatabin.getObject(tileHeaderDatabin);
        ...
        while (precinctIterator.tryAdvance()) {
            ...
            if (accumulatedData.qualityInTile !== undefined) {
                throw new jGlobals.jpipExceptions.InternalErrorException('Tile was ' + 'iterated twice in codestream part');
            }
    ...
    function precinctDataArrived(precinctDatabin) {
        ...
        var qualityInTile = accumulatedData.qualityInTile;
    ...
    function incrementPrecinctQualityLayers(precinctDatabin, accumulatedData, precinctIteratorOptional) {
        ...
        var prevCount = precinctCountByReachedQualityLayer[numQualityLayersReached] || 0;

        precinctCountByReachedQualityLayer[numQualityLayersReached] = prevCount + 1;
    }

    function tryAdvanceQualityLayersReached() {
        if (precinctCountByReachedQualityLayer[minNumQualityLayersReached] > 0 || minNumQualityLayersReached === 'max' || minNumQualityLayersReached >= numQualityLayersToWaitFor || tileHeadersNotLoaded > 0) {
    ...
    function ensureQualityLayersStatisticsForDebug() {
        ...
        for (var i = 0; i < registeredPrecinctDatabins.length; ++i) {
            var accumulatedData = accumulatedDataPerDatabin.getObject(registeredPrecinctDatabins[i]);
            ...
            var oldValue = precinctCountByReachedQualityLayerExpected[qualityLayers.numQualityLayers];

            precinctCountByReachedQualityLayerExpected[qualityLayers.numQualityLayers] = (oldValue || 0) + 1;
        }

        var length = Math.max(precinctCountByReachedQualityLayerExpected.length, precinctCountByReachedQualityLayer.length);
        ...
        for (var j = 0; j < length; ++j) {
            ...
            if (isExpectedZero !== isActualZero) {
                throw new jGlobals.jpipExceptions.InternalErrorException('Wrong accumulated statistics in JpipRequestDatabinsListener');
            }
            ...
            if (precinctCountByReachedQualityLayer[j] !== precinctCountByReachedQualityLayerExpected[j]) {
        ...
        if (minNumQualityLayersReached !== minNumQualityLayersReachedExpected) {
            throw new jGlobals.jpipExceptions.InternalErrorException('Wrong minNumQualityLayersReached in JpipRequestDatabinsListener');
        }
    }
};
...
module.exports = function JpipTileStructure(sizeParams, codestreamStructure, jpipFactory, progressionOrder) {
    ...
    function checkIfPrecinctPartitionStartsInTileTopLeft(resolutionLevel, numResolutionLevels, getPrecinctSizeFunction, getLevelSizeFunction, getTileSizeFunction) {
        ...
        var isPrecinctPartitionFitsToTilePartition = precinctSize % tileSize === 0 || tileSize % precinctSize === 0;
...
PdfjsJpxDecoder.prototype._copyTilesPixelsToOnePixelsArray = function copyTilesPixelsToOnePixelsArray(tiles, resultRegion, componentsCount) {
...
PdfjsJpxDecoder.prototype._copyTile = function copyTile(targetImage, tile, targetImageStartOffset, targetImageStride, componentsCount) {
